...
let gridW = Deno.args[0] && parseInt(Deno.args[0], 10) ? parseInt(Deno.args[0], 10) : SETTINGS.GRID_HEIGHT, gridH = Deno.args[1] && parseInt(Deno.args[1], 10) ? parseInt(Deno.args[1], 10) : SETTINGS.GRID_WIDTH, seedArg = Deno.args[2] ? parseSeedArg(parseInt(Deno.args[2], 10)) : parseSeedArg(new Date().getTime());
...
const configureCellularAutomata = (props)=>{
    const { gridW: argGridW = 10 , gridH: argGridH = 10 , seedAr: argSeedAr = new Date().getTime() , iterationsRemaining: argIterationsRemaining  } = props;
    console.log('-->', {
        argGridW,
        argGridH,
        argSeedAr,
...
export { configureCellularAutomata as configureCellularAutomata };
